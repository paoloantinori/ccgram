...
Follow-up (commit 8328623, same day): the deployed optimization never executed. asyncio.timeout CANCELS the in-flight anext at expiry, and the cancellation kills the stream generator (open_socket_stream's finally closes the socket), so the unchanged-mapping continue hit a dead stream and took the server-EOF path: reconnect + full re-prime every ~31s. The quiet-stream test was vacuous (the watcher stayed parked at its yield during the observation window), which is why it shipped green. Fix: pending-read shape (create_task'd anext + asyncio.wait, which does not cancel on expiry), cleanup in a finally INSIDE the aclosing block (closing a generator with a live driver raises RuntimeError), non-vacuous test (post-quiet event must arrive on the SAME stream; mutation-tested by the review agent). Dead reprime flags removed; ack bound 30s; _targets_moved unifies the move predicate. Deployed 4.9.4.dev4 and verified on the live bridge: herdr child forks fell from ~170/min (the 2.6 calls/s) to 6/min over a 65s quiet sample; bridge CPU unchanged at ~15.7% of one core because the eliminated forks spent their CPU in the child processes, not the parent.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Diagnosed (Errno 22: transient, era-correlated with the 2026-08-30 stress window, not reproducible via steady/churn/cancel paths, handled correctly by the reconnect loop) and fixed the adjacent efficiency defect it surfaced: reprime-after-coverage-loss-only with move detection, idle 30s, dedicated 5s ack timeout, EOF-graceful handling; three gate-found corrections applied with discriminating tests (pre-refresh event delivery, ack-phase transport path, first-connect prime preservation). 6730 tests green. Deployed 2026-08-31; the keep-stream-open mechanism itself was fixed the same day in 8328623 (asyncio.timeout had been killing the stream at every idle expiry; verified live: herdr forks ~170/min to 6/min).
